...
var startTest = async function (netClient) {
  ...
  function getPuppetSubscriptionsGetPermissions (substitutions = {}) {
    var defaultSubscription = Object.assign({
      id: 'testSubscription',
      permissions: ['dashboardsWrite', 'dashboardsRead', 'dashboardsConfirm', 'dashboardsWriteOtherUsers', 'dashboardsReadHidden'],
      dashId: 'testDash',
      userId: 'testUser'
    }, substitutions)
  ...
  const dbGet = (id = 'testDashboard', bucket = 'dashboardsViews') => DB.get(bucket, id)
  const dbRemove = (id = 'testDashboard', bucket = 'dashboardsViews') => DB.remove(bucket, id)
  ...
  test = await netClient.testLocalMethod('createMulti', context.data, {token: context.tokens.userTest})
  mainTest.testRaw('createMulti checkError No permission to write dashboards', test, (data) => data.errors instanceof Array)
  await dbRemove(test.results[0].id)

  context.updatePuppets({subscriptions_getPermissions: getPuppetSubscriptionsGetPermissions({'permissions': ['dashboardsWrite']})})
  test = await netClient.testLocalMethod('createMulti', context.data, {token: context.tokens.userTest})
  ...
  context = await setContext({
    ...
    users: { userTest: {} },
    entities: [
      { id: 'testDashboard', dashId: 'testDash', userId: 'userTest', body: 'test', tags: ['testTag'], toTags: ['testTag'], toRoles: ['subscriber'] },
      { id: 'testDashboard1', dashId: 'testDash', userId: 'userTest', body: 'test', tags: ['testTag'], toTags: ['testTag'], toRoles: ['subscriber'] }
    ],
    testPuppets: { subscriptions_getPermissions: getPuppetSubscriptionsGetPermissions() }
  })
  test = await netClient.testLocalMethod('readMulti', { ids: ['testDashboard'] }, {token: context.tokens.userTest})
  ...
  context = await setContext({
    ...
    entities: [
      { id: 'testDashboard', dashId: 'testDash', userId: 'userTest', body: 'test', tags: ['testTag'], toTags: ['testTag'], toRoles: ['subscriber'] },
      { id: 'testDashboard1', dashId: 'testDash', userId: 'userTest1', body: 'test', tags: ['testTag'], toTags: ['testTag'], toRoles: ['subscriber'] }
    ],
    testPuppets: { subscriptions_getPermissions: getPuppetSubscriptionsGetPermissions({'permissions': ['dashboardsWrite', 'dashboardsRead']}) }
  })

  test = await netClient.testLocalMethod('updateMulti', { items: [{id: 'testDashboard', tags: ['testUpdate']}] }, {token: context.tokens.userTest})
  mainTest.testRaw('updateMulti', test, (data) => !data.errors && data.results instanceof Array && data.results.length === 1)
  // dbCheck = await DB.get('dashboardsViews', 'testDashboard')
  mainTest.testRaw('updateMulti dbCheck', await dbGet('testDashboard'), (data) => data.tags[0] === 'testUpdate')
  ...
  test = await netClient.testLocalMethod('updateMulti', { items: [{id: 'testDashboard1', tags: ['test']}] }, {token: context.tokens.userTest})
  mainTest.testRaw('updateMulti checkError  Cant update other users dashboard', test, (data) => data.errors instanceof Array)

  context.updatePuppets({subscriptions_getPermissions: getPuppetSubscriptionsGetPermissions({'permissions': ['dashboardsWrite', 'dashboardsRead', 'dashboardsWriteOtherUsers']})})
  test = await netClient.testLocalMethod('updateMulti', { items: [{id: 'testDashboard1', tags: ['test']}] }, {token: context.tokens.userAdminTest})
  mainTest.testRaw('updateMulti checkError  dashboardsWrite can update other users dashboard', test, (data) => !data.errors)
  ...
  context = await setContext({
    ...
    entities: [
      { id: 'testDashboard', dashId: 'testDash', userId: 'userTest', body: 'test', tags: ['testTag'], toTags: ['testTag'], toRoles: ['subscriber'] },
      { id: 'testDashboard1', meta: {deleted: true}, dashId: 'testDash', userId: 'userTest1', body: 'test', tags: ['testTag'], toTags: ['testTag'], toRoles: ['subscriber'] }
    ],
    testPuppets: { subscriptions_getPermissions: getPuppetSubscriptionsGetPermissions({'permissions': ['dashboardsWrite', 'dashboardsRead']}) }
  })

  test = await netClient.testLocalMethod('deleteMulti', { ids: ['testDashboard'] }, {token: context.tokens.userTest})
  ...
  mainTest.testRaw('deleteMulti dbCheck', await dbGet('testDashboard'), (data) => data.meta.deleted === true)

  test = await netClient.testLocalMethod('readMulti', { ids: ['testDashboard1'] }, {token: context.tokens.userTest})
  mainTest.testRaw('readMulti checkError readMulti Dashboard deleted', test, (data) => data.errors instanceof Array)

  test = await netClient.testLocalMethod('readMulti', { ids: ['testDashboard'] }, {token: context.tokens.userTest})
  mainTest.testRaw('readMulti check Dashboard deleted but readable by owner', test, (data) => !data.errors)

  context.updatePuppets({subscriptions_getPermissions: getPuppetSubscriptionsGetPermissions({'permissions': ['dashboardsWrite', 'dashboardsRead', 'dashboardsReadHidden', 'dashboardsWriteOtherUsers']})})
  test = await netClient.testLocalMethod('readMulti', { ids: ['testDashboard1'] }, {token: context.tokens.userTest})
  ...
  test = await netClient.testLocalMethod('addTagsMulti', { items: [{id: 'testDashboard', tags: ['tag_by_subscriber', 'tag_by_subscriber2']}] }, {token: context.tokens.userTest})
  ...
  mainTest.testRaw('addTagsMulti by subcritpion owner dbCheck', await dbGet('testDashboard'), (data) => data.tags.indexOf('tag_by_subscriber') > -1)

  test = await netClient.testLocalMethod('addTagsMulti', { items: [{id: 'testDashboard', tags: ['tag_by_admin', 'tag_by_admin2']}] }, {token: context.tokens.userAdminTest})
  mainTest.testRaw('addTagsMulti by admin', test, (data) => !data.errors && data.results instanceof Array && data.results.length === 1)
  mainTest.testRaw('addTagsMulti by admin  dbCheck', await dbGet('testDashboard'), (data) => data.tags.indexOf('tag_by_admin') > -1)
