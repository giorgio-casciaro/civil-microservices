<template>
<form class="Login" @click="active=true" @submit.prevent="waiting=true;action_rpcCallAndStoreMutation({rpcMethod:'login',mutation:'LOGIN',data:form,success:succ,error:err})" @input="validation=getter_rpcValidate({rpcMethod:'login',data:form})" :class="{validForm:validation.valid,activeForm:active}">
  ...
  <input type="submit" class="login button" :disabled="waiting" :class="{error,success,waiting}" :value="getter_t('strLogin')">
  <label class="rememberMe"><input type="checkbox" :disabled="waiting" v-model="rememberMe" />{{getter_t('strRememberMe')}}</label>
