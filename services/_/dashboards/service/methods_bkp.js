...
var service = function getMethods (CONSOLE, netClient, CONFIG = require('./config')) {
  try {
    ...
    return {
      ...
      async subscriptionsListLast (reqData = {}, meta = {directCall: true}, getStream = null) {
        ...
        CONSOLE.hl('subscriptionsListLast', dashId, dashSubscriptionsNumber, userId, rawIds)
        // return {}
        // await subscriptionCan(reqData.dashId, userId, 'postsReads')
        var results = await Promise.all(rawIds.map((id) => subscriptionsRead(id, userId, subscription)))
        CONSOLE.hl('subscriptionsListLast results', results)
      ...
      postsListLast: posts.postsListLast,
