...
var startTest = async function (netClient) {
  ...
  var longRpc = async () => {
    ...
    mainTest.testRaw('SERVICE RPC', {rpcCounter, rpcOkCounter, testEventsCCounter, Seconds: (Date.now() - rpcStartTime) / 1000, millisecondsPerRequest: (Date.now() - rpcStartTime) / rpcCicles}, (data) => data.rpcOkCounter === data.testEventsCCounter)
